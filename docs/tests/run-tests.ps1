<#
PowerShell smoke test script to validate protected endpoints.
Usage:
  $env:API_URL = 'https://api.example.com'
  $env:TEST_EMAIL = 'test@example.com'   # optional; if missing an ephemeral email will be used
  $env:TEST_PASSWORD = 'password'        # required if TEST_EMAIL provided
  powershell -File .\run-tests.ps1

This script performs a small number of checks and will create an ephemeral test user
when TEST_EMAIL is not set. Ephemeral users are deleted at the end of the run.
#>
...
# Unauthenticated checks (inventory/mechanics currently allow unauth POSTs; service-tickets do not)
Write-Host "`n[Unauthenticated] POST /inventory -> expect success (201)"
$invUna = Invoke-Json -method 'POST' -url "$apiUrl/inventory/" -body @{ name = 'ps-smoke' ; price = 1.23 }
if ($invUna.ok) { Write-Host 'OK POST /inventory (unauth): succeeded as expected' } else { Write-Host "FAIL POST /inventory (unauth): expected success but got $($invUna.status)" }

Write-Host "`n[Unauthenticated] POST /mechanics -> expect success (201)"
...
if ($mechUna.ok) { Write-Host 'OK POST /mechanics (unauth): succeeded as expected' } else { Write-Host "FAIL POST /mechanics (unauth): expected success but got $($mechUna.status)" }

Write-Host "`n[Unauthenticated] POST /service-tickets -> expect unauthorized (401/403/404)"
$ticketUna = Invoke-Json -method 'POST' -url "$apiUrl/service-tickets/" -body @{ customer_id = 1; description = 'ps smoke test' }
if (-not $ticketUna.ok) { Write-Host "OK POST /service-tickets (unauth): got $($ticketUna.status) (unauthenticated)" } else { Write-Host 'FAIL POST /service-tickets (unauth): unexpected success' }
...
if (-not $email) {
    ...
    Write-Host "`nNo TEST_EMAIL provided - using ephemeral email: $email"
} elseif (-not $password) {
    Write-Host "`nTEST_EMAIL provided but TEST_PASSWORD missing; cannot proceed with authenticated checks"
    exit 0
}
...
if (-not $login.ok -or -not $login.body.token) {
    ...
    Write-Host "Test user created (id $createdCustomerId), retrying login..."
    $createdUser = $true
    $login = Invoke-Json -method 'POST' -url "$apiUrl/customers/login" -body @{ email = $email; password = $password }
    if (-not $login.ok -or -not $login.body.token) {
...
Write-Host "`n[Auth] POST /inventory/"
...
Write-Host "`n[Auth] POST /mechanics/"
...
Write-Host "`n[Auth] POST /service-tickets/"
...
if ($createdUser) {
    Write-Host "`nCleaning up test user..."
    $cid = $createdCustomerId
    if (-not $cid -and $login.body.customer) { $cid = $login.body.customer.id }
    if ($cid) {
        Invoke-Json -method 'DELETE' -url "$apiUrl/customers/$cid" -headers $headers | Out-Null
        Write-Host 'Test user deleted'
    } else { Write-Host 'Could not determine customer id to delete' }
}

Write-Host "`nPS smoke tests completed. If all steps above succeeded you have both unauth/auth behavior confirmed."
<#
